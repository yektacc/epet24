<?php namespace App\Http\Controllers;
	...
	class AdminSrvCenterAnswersController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->table = "srv_center_answers";
			# END CONFIGURATION DO NOT REMOVE THIS LINE

			# START COLUMNS DO NOT REMOVE THIS LINE
			$this->col = [];
			$this->col[] = ["label"=>"نام مرکز","name"=>"center_department_id","join"=>"srv_centers,center_name"];
			$this->col[] = ["label"=>"توضیحات","name"=>"center_department_id","join"=>"srv_centers,center_description"];
...
//			$this->col[] = ["label"=>"نام منطقه","name"=>"center_department_id","join"=>"srv_center_addresses,district_name"];
			$this->col[] = ["label"=>"سوال","name"=>"question_id","join"=>"srv_questions,question_text"];
			$this->col[] = ["label"=>"پاسخ","name"=>"center_answer","join"=>"answers,answer_text"];
			...
			$this->form[] = ['label'=>'سوال','name'=>'question_id','type'=>'select2','validation'=>'required','width'=>'col-sm-10','datatable'=>'srv_questions,question_text'];
			$this->form[] = ['label'=>'پاسخ','name'=>'center_answer','type'=>'select2','validation'=>'required','width'=>'col-sm-10','datatable'=>'answers,answer_text'];
...
//			$me = CRUDBooster::myName();
	    ...
	    */
	    public function hook_after_add($id) {        
			...
			$answer_id = DB::table('srv_center_answers')
			//->where('srvcenter_sellerid',$app_seller_id)
			->orderby('id','DESC')
			->pluck('id')
			->first();

			DB::table('srv_center_answers')
			->where('id',$answer_id)
