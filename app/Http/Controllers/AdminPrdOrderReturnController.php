<?php namespace App\Http\Controllers;
	...
	class AdminPrdOrderReturnController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col = [];
			$this->col[] = ["label"=>"تاریخ سفارش","name"=>"order_id","join"=>"prd_orders,order_date"];
			$this->col[] = ["label"=>"کد سفارش","name"=>"order_item_id","join"=>"shop_order_items,order_code"];
			$this->col[] = ["label"=>"نام فروشنده","name"=>"order_item_id","join"=>"shop_order_items,product_seller"];
			$this->col[] = ["label"=>"نام خریدار","name"=>"app_user_id","join"=>"app_users,firstname"];
			$this->col[] = ["label"=>"نام خانوادگی خریدار","name"=>"app_user_id","join"=>"app_users,lastname"];
//			$this->col[] = ["label"=>"شهر فروشنده","name"=>"order_item_id","join"=>"shop_order_items,shop_city"];
			$this->col[] = ["label"=>"نام محصول","name"=>"order_item_id","join"=>"shop_order_items,product_title"];
			$this->col[] = ["label"=>"توضیحات","name"=>"comment"];

//			$this->col[] = ["label"="قیمت","name"=>""];
...
//			$this->form[] = ['label'=>'Order Item Id','name'=>'order_item_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'order_item,id'];
//			$this->form[] = ['label'=>'App User Id','name'=>'app_user_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'app_user,id'];
//			$this->form[] = ['label'=>'Srv Center Id','name'=>'srv_center_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'srv_center,id'];
			$this->form[] = ['label'=>'توضیح فروشنده','name'=>'seller_comment','type'=>'textarea','validation'=>'required|string|min:5|max:5000','width'=>'col-sm-10'];
			...
			//$this->form[] = ['label'=>'App User Id','name'=>'app_user_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'app_user,id'];
			//$this->form[] = ['label'=>'Srv Center Id','name'=>'srv_center_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'srv_center,id'];
			//$this->form[] = ['label'=>'Comment','name'=>'comment','type'=>'textarea','validation'=>'required|string|min:5|max:5000','width'=>'col-sm-10'];
			# OLD END FORM
